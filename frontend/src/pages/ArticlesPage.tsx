...
type Article = { _id: string; slug?: string; title: string; content: string; views: number; likes?: any[]; createdAt: string; author?: { name?: string } }
...
export default function ArticlesPage() {
  ...
  async function load() {
    setLoading(true)
    try {
  const { data } = await axios.get('/api/articles', { params: { sort, q, page, limit } })
  setItems(data.items || [])
  ...
  <button className="btn" onClick={() => { setPage(1); load() }}>Search</button>
      ...
      return (
        ...
        <Link to={`/articles/${it.slug || it._id}`} className="font-semibold text-lg hover:underline block truncate">{it.title}</Link>
                ...
                <Link to={`/articles/${it.slug || it._id}`} className="mt-2 text-neutral-200 line-clamp-2 block">{it.content.slice(0, 240)}</Link>
              </div>
            )
          })}
      ...
      {total > limit && (
        <div className="mt-4 flex items-center justify-center gap-2">
          ...
          <div className="text-sm text-neutral-400">Page {page} of {Math.ceil(total/limit)}</div>
          <button className="btn" disabled={page>=Math.ceil(total/limit)} onClick={() => setPage(p=>p+1)}>Next</button>
        </div>
      )}
    </div>
  )
